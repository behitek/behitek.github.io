...
<div class="rgrid grid grid-cols-[3fr_1fr] bg-bg">
  <div class="rpad px-12 pt-[72px] pb-14 border-r-2 border-ink">
    <div class="kicker mb-[18px]">hieu@behitek:~ $ whoami</div>
    <h1 class="font-heading font-extrabold text-[44px] md:text-[62px] leading-[1.02] text-ink mb-2 tracking-tight">
      Hiếu Nguyễn
    </h1>
    <div class="font-heading font-bold text-2xl text-accent mb-5">
      AI Engineer · Kiến trúc giải pháp · Hà Nội
    </div>
    <p class="text-[17px] leading-relaxed text-neutral-700 mb-8 max-w-[720px]" style="text-wrap: pretty">
      Tôi xây hệ thống AI chạy thật trong production — và vận hành hạ tầng luyện code miễn phí cho
      {ORGANIZATIONS.length}+ trường học trên khắp Việt Nam, từ THPT chuyên đến trường nội trú vùng cao.
    </p>
  ...
  <div class="p-10 flex flex-col gap-6">
    <img
      src="/images/me.png"
      alt="Hiếu Nguyễn"
      class="w-full aspect-square object-cover block border-2 border-ink grayscale-photo"
    />
...
<div class="rgrid grid grid-cols-4 border-t-2 border-ink">
  {
    STATS.map((stat, i) => (
      <div class={`px-12 py-[22px] ${i < STATS.length - 1 ? 'border-r-2 border-ink' : ''}`}>
        <div class="font-heading font-extrabold text-[30px] text-ink">{stat.value}</div>
...
<div class="rgrid grid grid-cols-[1.3fr_1fr] bg-white border-t-2 border-b-2 border-ink">
  <div class="rpad rwrap px-12 py-5 border-r-2 border-ink flex items-baseline gap-4">
    <span class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] flex-none">
      HIỆN TẠI
    </span>
    <span class="font-heading font-bold text-base text-ink">{CURRENT.org}</span>
    <span class="font-mono text-[13px] text-neutral-600">{CURRENT.detail}</span>
  </div>
  <div class="rpad rwrap px-12 py-5 flex items-baseline gap-4">
    <span class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] flex-none">
      TRƯỚC ĐÓ
    </span>
    <span class="font-heading font-bold text-base text-ink">{PAST}</span>
...
<div class="rgrid grid grid-cols-4 border-b-2 border-ink bg-bg">
  {
    AWARDS.map((award, i) => (
      <div class={`px-12 py-[22px] ${i < AWARDS.length - 1 ? 'border-r-2 border-ink' : ''}`}>
        <div class="font-heading font-extrabold text-[15px] text-accent mb-1">{award.rank}</div>
...
<div class="rgrid grid grid-cols-3 border-2 border-ink bg-white">
  {
    SKILLS.map((skill, i) => (
      <div class={`p-7 flex flex-col ${i < SKILLS.length - 1 ? 'border-r-2 border-ink' : ''}`}>
        <h3 class="font-heading font-bold text-[21px] leading-tight text-ink mb-2.5">{skill.title}</h3>
        <p class="font-mono text-[13px] leading-[1.9] text-neutral-700 mb-4">{skill.areas}</p>
        <div class="flex flex-wrap gap-1.5 mb-5">
          {skill.tech.map((t) => (
            <span class="tag-pill">{t}</span>
          ))}
        </div>
        <div class="mt-auto border-t-2 border-ink pt-3.5 font-heading font-bold text-[13px] leading-[1.5] text-accent">
          → {skill.proof}
        </div>
      </div>
    ))
...
```astro
  <section class="p-12 pb-0">
    <div class="flex items-baseline gap-4 mb-7">
      ...
      <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Chuyên môn</h2>
    </div>
    <SkillsGrid />
  </section>
```
...
<div>
  <div class={`rgrid grid ${cols} border-t-2 border-ink`}>
    {
      orgs.map((org) => (
        <div class="flex items-baseline gap-2 py-2 pr-6 border-b border-border-light font-mono text-[12.5px] leading-[1.5] text-ink">
          <span class="text-accent flex-none">●</span>
          <span class="flex-1">{org.name}</span>
          {variant === 'full' && <span class="text-neutral-600 flex-none">{org.members}</span>}
        </div>
      ))
    }
  </div>
  <a
    ...
    class="inline-block mt-3.5 font-mono text-[13px] font-semibold text-ink no-underline hover:text-accent"
  >
...
<div class="border-2 border-ink bg-white">
  <div class="rgrid grid grid-cols-[1.2fr_1fr]">
    <div class="p-8 border-r-2 border-ink">
      <div class="font-mono text-xs text-neutral-600 mb-2.5">{kicker}</div>
      <h3 class="font-heading font-bold text-2xl leading-tight text-ink mb-3">{title}</h3>
      <p class="text-[14.5px] leading-relaxed text-neutral-700 mb-5">{description}</p>
      <div class="rwrap flex gap-7 mb-5">
        {
          stats.map((s) => (
            <div>
              <div class="font-heading font-extrabold text-[26px] text-ink">{s.value}</div>
      ...
      </div>
      {
        links && links.length > 0 && (
          <div class="rwrap flex gap-5 font-mono text-[13px] font-semibold border-t-2 border-ink pt-4">
            {links.map((link) => (
              <a
                href={link.href}
                target="_blank"
    ...
    <div class="flex flex-col bg-bg">
      {image && <img src={image} alt={title} class="w-full h-full object-cover object-top block" />}
      {
        hasQuote && (
          <div class={`p-8 ${image ? 'border-t-2 border-ink' : 'h-full flex flex-col justify-between'}`}>
            <p class="font-medium text-[17px] leading-[1.55] text-ink m-0">“{quote}”</p>
            <div class="font-mono text-xs text-neutral-600 mt-4">— {attribution}</div>
          </div>
        )
      }
    </div>
...
import { ORGANIZATIONS, DEPLOYMENTS, TOTAL_ORG_MEMBERS } from '@/data/organizations';
```
    ...
    flagshipProject && (
      <section class="px-12 pb-14">
        <div class="flex items-baseline gap-4 mb-7">
          ...
          <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Case study nổi bật</h2>
        </div>
        <CaseStudySpotlight
        ...
        >
          <div class="border-t-2 border-ink p-8">
            <div class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] mb-4">
              {ORGANIZATIONS.length} TRƯỜNG & TỔ CHỨC ĐANG SỬ DỤNG — {TOTAL_ORG_MEMBERS.toLocaleString('vi-VN')} THÀNH VIÊN
            </div>
            <OrgRoster />
          </div>
        </CaseStudySpotlight>
...
const { title, description, tech, status, href, borderRight = false, borderBottom = false } = Astro.props;
...
>
  <div class="flex justify-between items-center mb-3">
    <span class="font-mono text-[10px] font-semibold border border-ink text-ink px-[7px] py-0.5">
      {statusLabel}
    </span>
  </div>
  <h3 class="font-heading font-bold text-[21px] leading-tight text-ink mb-2.5">{title}</h3>
  <p class="text-sm leading-relaxed text-neutral-700 mb-4">{description}</p>
  ...
  </div>
  {
    href && (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        class="font-heading font-bold text-[13px] text-accent no-underline"
      >
        Xem chi tiết →
      </a>
    )
...
const otherProjects = AI_FIRST_ORDER
  .map((id) => allProjects.find((p) => p.data.id === id))
      ...
      <section class="px-12 pb-14">
        <div class="flex items-baseline justify-between mb-7">
          <div class="flex items-baseline gap-4">
            ...
            <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Dự án</h2>
          </div>
          <a href="/projects" class="font-mono text-[13px] font-semibold text-ink no-underline hover:text-accent">
            ls ./projects →
          </a>
        </div>
...
<div class="rgrid grid grid-cols-3 border-2 border-ink">
  {
    TIERS.map((tier, i) => (
      <div
        class={`p-7 flex flex-col bg-white ${i < TIERS.length - 1 ? 'border-r-2 border-ink' : ''} ${
          tier.highlighted ? 'border-t-4 border-t-accent' : ''
        }`}
      >
        <div class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] mb-2.5">
          {tier.kicker}
        </div>
        <h3 class="font-heading font-bold text-xl leading-tight text-ink mb-1.5">{tier.title}</h3>
        <div class="font-heading font-extrabold text-[22px] text-accent mb-3.5">{tier.price}</div>
        <p class="text-[13.5px] leading-relaxed text-neutral-700 mb-5">{tier.description}</p>
...
```astro
    <div class="flex items-baseline gap-4 mt-12 mb-7">
      <span class="kicker">01 /</span>
      <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Ba cách bắt đầu</h2>
    </div>
    <ServiceTiers />
```
...
<div class="border-t-2 border-ink">
  {
    items.map((item) => (
      <details class="border-b-2 border-ink group">
        <summary class="cursor-pointer list-none py-4 flex items-baseline gap-3 font-heading font-bold text-[17px] text-ink hover:text-accent">
          <span class="font-mono text-accent flex-none">+</span>
          {item.question}
        </summary>
        <p class="text-[14.5px] leading-relaxed text-neutral-700 pb-5 pl-6 m-0 max-w-[760px]">{item.answer}</p>
      </details>
    ))
  }
</div>
...
const FAQ_ITEMS = [
  ...
  {
    question: 'Đang dùng gói miễn phí, sau này muốn chuyển sang hệ thống riêng thì có mất dữ liệu không?',
    answer:
...
```astro
  <section class="px-12 pb-12">
    <div class="flex items-baseline gap-4 mb-7">
      ...
      <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Đang chạy ở đâu</h2>
    </div>
    <div class="border-2 border-ink bg-white p-8">
      <div class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] mb-4">
        {ORGANIZATIONS.length} TỔ CHỨC TRÊN LUYENCODE.NET — {TOTAL_ORG_MEMBERS.toLocaleString('vi-VN')} THÀNH VIÊN
      </div>
      <OrgRoster variant="full" />
      <div class="font-mono text-[11px] font-semibold text-neutral-600 tracking-[0.08em] mt-8 mb-4">
      ...
      <div class="border-t-2 border-ink">
        {
          DEPLOYMENTS.map((d) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              class="flex items-baseline gap-3 py-3 border-b border-border-light font-mono text-[13px] text-ink no-underline hover:text-accent"
            >
              <span class="text-accent flex-none">●</span>
...
```astro
  <section class="px-12 pb-12">
    <div class="flex items-baseline gap-4 mb-7">
      <span class="kicker">04 /</span>
      <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Ngân hàng đề sẵn có</h2>
    </div>
    <div class="rgrid grid grid-cols-[1fr_1.2fr] border-2 border-ink bg-white">
      <div class="p-8 border-r-2 border-ink">
        <div class="font-heading font-extrabold text-[46px] leading-none text-accent mb-2">900+</div>
        <div class="font-mono text-[13px] text-neutral-600">bài tập kèm lời giải, dùng được ngay</div>
      </div>
      <div class="p-8">
        <p class="text-[14.5px] leading-relaxed text-neutral-700 m-0">
...
```astro
  <section class="px-12 pb-12">
    <div class="flex items-baseline gap-4 mb-7">
      ...
      <h2 class="font-heading font-extrabold text-[34px] text-ink m-0">Xem thử hệ thống</h2>
    </div>
    <div class="border-2 border-ink bg-white p-8">
...
<div class={`rgrid grid ${TESTIMONIALS.length > 0 ? 'grid-cols-2' : 'grid-cols-1'} border-t-2 border-ink bg-white`}>
  <div class={`rpad px-12 py-9 ${TESTIMONIALS.length > 0 ? 'border-r-2 border-ink' : ''}`}>
    <h2 class="font-heading font-extrabold text-[26px] text-ink mb-4.5">Đơn vị đang vận hành</h2>
    <div class="flex flex-col gap-3 font-mono text-sm text-ink">
      {
        clients.map((c) => (
          <a href={c.url} target="_blank" rel="noopener noreferrer" class="text-ink no-underline hover:text-accent">
            <span class="text-accent">●</span> {c.name} <span class="text-neutral-600">— {c.note}</span>
          </a>
        ))
      }
    </div>
    ...
    TESTIMONIALS.length > 0 && (
      <div class="bg-bg flex flex-col">
        {TESTIMONIALS.map((t, i) => (
          <div class={`px-12 py-9 flex-1 ${i > 0 ? 'border-t-2 border-ink' : ''}`}>
            <p class="font-medium text-[17px] leading-[1.55] text-ink mb-3.5">“{t.quote}”</p>
